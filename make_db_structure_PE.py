...
#Anmerkung: zur Erstellung der Phosphoreintrag-Datenbank-Struktur wird auf die Sedimenteintragsraster und Vektoren zurückgegriffen
# (hierfür hatte ich schon ein Datenbank-Skript erstellt) -> dies funktioniert aber da zu jedem Sedimenteintrag
# ein Phosphoreintrag korrespondiert
...
#(in this Folder the subfolders "Frienisberg", "Lobsigen", "Schwanden", "Seedorf", "Suberg" already existed...if not
#they have to be coded or created quickly manually... )
...
#################################### create folders like Frienisberg_1997_2007/Jahr/1998/Mittel #####################
...
for vec_gb_p in origin_list:
    ...
    # make new subdirectories Datum/year/Mittel in e.g this path Sedimenteintrag\Frienisberg\Frienisberg_1997_2007 and similar ones
    for geb in os.listdir(Gebiete):
        path_geb = os.path.join(Gebiete, geb)
        os.chdir(path_geb)
        ...
        for geb_p in os.listdir(path_geb):
            ...
            # if geb_p_name (e.g. from Sedimenteintrag/Seedorf/Seedorf_2007_2017, it's Seedorf_07_17) is equal to basename of SE_vectors_copied folder, then create
            # new directories -> Sedimenteintrag/Seedorf/Seedorf_2007_2017  new is /Jahr/Jahr(e.g.2008)/Mittel
            if geb_p_name == os.path.basename(vec_gb_p):
                for d in year_List:
                    if os.path.exists(os.path.join(path_geb_p, "Jahr", d, "Mittel")):
                        print("path already exists")
                    else:
                        new_dir = os.makedirs(os.path.join(path_geb_p, "Jahr", d, "Mittel"))
